...
// Middleware (urlencoded so HTML <form> POSTs work; json for fetch/API)
...
app.post("/api/cars", (req, res) => {
  ...
  const { brand, make, year, color } = body;
  ...
  if (
    ...
    return res.status(400).json({
      error: "Missing fields. Send JSON (Content-Type: application/json) or use a form with brand, make, year, color."
    });
  }
